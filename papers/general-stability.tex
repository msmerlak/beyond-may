...
With more general interactions coefficients $A = (a_{ij})$, the Jacobian of the system \eqref{dynamics} evaluated at a putative fixed point $x_*$ reads
\end{eqnarray}
\begin{equation}
    J_* = \mathcal{D}(f'_* - f_*g'_*/g_*) + \mathcal{D}(g_*)A\mathcal{D}(h'_*)
\end{equation}
Let us now consider the case where $a_{ij}$ are i.i.d. with mean $\mu$ and standard deviation $\sigma$. 
