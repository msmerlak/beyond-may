...
$x_*^{\alpha}$. In the following, in studying a system which
follows in the 'anti-May' phase, we show that numerical results
obtained for the case in which the diagonal part of the
interaction matrix is non-vanishing and equal to the
...
In this case the first and second moment to be self-consistently
computed formally diverges. This is not truly porblematic in practice
...
we can Taylor expand to the first-order the cavity 
solution~\eqref{eq: cavity solution} and obtain a
gaussian approximation to Eq.~\eqref{eq: dist general}
with moments $\langle x_*\rangle=(\mu N)^{-2/3}$ and
$\langle x_*^2\rangle=(\mu N)^{-4/3}/(1-4(\mu N)^{-2}\sigma^2N)$.
This is plotted against simulations in 
Fig.~\ref{fig: cavity sol.}, alongside the exact solution~\eqref{eq: dist general} 
where integration for the moments has been performed up to a maximum
value above which no equilibirum value should be found for a given $N$.
...
in it if already stable. This confirms we are in the
'anti-May' phase.
