...
\begin{equation}
    \dot{x} = x^{\alpha}-A_sx^{\beta+\gamma}-x^{\beta}\big( \tilde{\mu} \langle x^{\gamma}\rangle + \tilde{\sigma} \eta\big) \, ,
\label{eq: dmft}
\end{equation}
...
\begin{equation}
    x_*^{\alpha-\beta} - A_s x_*^{\gamma}= \big( \tilde{\mu} \langle x_*^{\gamma}\rangle + \tilde{\sigma} \sqrt{\langle x_*^{2\gamma}\rangle}\xi\big) \, ,
\end{equation} 
