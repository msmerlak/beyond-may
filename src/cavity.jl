...
function P_n(n, e1_n, e2_n, p)
    ...
    x = max(n,1e-20)
    return abs(α-β)*x^(α-β-1)/(sqrt(2*π*σ^2*e2_n))*exp(-(x^(α-β)-μ*e1_n)^2/(2*σ^2*e2_n))
end
...
function Cavity_Λ(p;
    ...
    Λ_init = 1, #initial guess for cut-off
    ...
    for i in 1:Iter
    #    ϕ_new = first(quadgk(x -> P_n(x, e1_n[i], e2_n[i], p) , n_min, Λ_n[i], rtol=tol))
    ...
    ϕ_n_eq, e1_n_eq, e2_n_eq, Λ_n_eq = ϕ_n[Iter], e1_n[Iter], e2_n[Iter], Λ_n[Iter]
    return (ϕ_n_eq, e1_n_eq, e2_n_eq, Λ_n_eq)
end

#= cavity calculation for fraction of positive equilibriums 
and first two moments of the equilibrium distribution given μ and σ.
In this case we keep Λ=n_max constant to speed up computations and vaoid to 
find the right ansatz for initial condition case by case.
...
#= Gaussian approximation for the distribution and critical line for this case. =#
